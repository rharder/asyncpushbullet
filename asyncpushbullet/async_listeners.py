...
class PushListener2(object):
    ...
    async def __aenter__(self):
        ...
        async def _listen_for_pushes():
            # headers = {"Access-Token": self.account.api_key}
            try:
                # session = await self.account.aio_session()
                # async with WebsocketClient(url=WEBSOCKET_URL + self.account.api_key,
                #                            proxy=self.account._proxy,
                #                            session=session) as wc:
                #     self.ws_client = wc
                async for msg in wc:
                    # print("__aenter__ msg:", msg)
        ...
        wc = WebsocketClient(url=WEBSOCKET_URL + self.account.api_key,
                                           proxy=self.account._proxy,
                                           session=session)
        self.ws_client = await wc.__aenter__()
        asyncio.create_task(_listen_for_pushes())
        await asyncio.sleep(0)
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        await self.ws_client.__aexit__(exc_type, exc_val, exc_tb)
        await self.ws_client.close()
    ...
    async def next_push(self, timeout: int = None):
        if timeout is None:
